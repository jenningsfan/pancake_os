...
// TODO: support for channel 2 properly
...
impl PS2Controller {
    ...
    pub fn enable_port1(&mut self, irq: bool) {
        unsafe {
            self.write_command(Command::EnablePort1 as u8);
    ...
    pub unsafe fn read_and_wait(&mut self) -> u8 {
        unsafe {
    ...
    pub unsafe fn write_command_device(&mut self, command: u8) {
        // poll until input buffer is clear
        unsafe {
            self.wait_for_input_buffer_write();
    ...
    pub unsafe fn write_command_val_device(&mut self, command: u8, value: u8) {
        unsafe {
            self.wait_for_input_buffer_write();
            self.data_port.write(command);
